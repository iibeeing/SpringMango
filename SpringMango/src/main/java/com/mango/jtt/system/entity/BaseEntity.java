...
public abstract class BaseEntity implements Serializable {
	...
	*/ 
	...
	private static Map<Class<?>, PropertyInfo[]> class2Props = new HashMap<Class<?>, PropertyInfo[]>(
			128);
			...
			try {
				...
				if ((method.getModifiers() & Modifier.PUBLIC) == 1
					...
					if (methodName.startsWith("get")
						...
						if (methodName.startsWith("get")) {
							propInfo.propName = methodName.substring(3, 4)
									.toLowerCase() + methodName.substring(4);
						} else if (methodName.startsWith("is")) {
							propInfo.propName = methodName.substring(2, 3)
